...
- `providers/` — `ScannerProvider` abstraction (discover/canParse/reduce/finalize). `ThreadbaseProvider` wraps the shared `metadata-reducer` (no duplication); `CodexCliProvider` parses local OpenAI Codex CLI rollout sessions; `CursorProvider` parses Cursor agent-transcripts JSONL (`~/.cursor/projects/<slug>/agent-transcripts/`) into the same `ConversationMeta` model

### Providers (`src/providers/`)

Both the Claude/Threadbase format, local Codex CLI history, and Cursor agent-transcripts flow through one normalized provider pipeline. Codex and Cursor are **opt-in** via `scan({ providers: ['claude-code', 'codex-cli', 'cursor'], codexRoots: [...], cursorRoots: [...] })` — no home directory is scanned by default; `*Roots` must be absolute. The Cursor wire name is **`cursor`** (same as streamer/mobile); `cursor-cli` is accepted as an alias. Not `cursor-agent`. Composer `state.vscdb` is out of scope. Adding another agent: [`.claude/skills/add-provider/SKILL.md`](.claude/skills/add-provider/SKILL.md). That skill is paired with the same path in `threadbase-streamer` and `threadbase-mobile` — change all three together.

**Codex and Cursor are indexed in both the in-memory and SQLite persistent engines.** A persistent-mode scan/search with the matching `providers` + `*Roots` discovers those files through their `ScannerProvider`, reduces them via the shared provider pipeline, and upserts them into the same `conversations`/FTS tables as Threadbase. Threadbase files keep the byte-offset-resumable incremental fold; Codex and Cursor files reparse from offset 0 on each change (sessions are small — see the `ponytail:` note in `index-engine.ts` for the upgrade path). Persisted rows carry a `provider` column (schema v3); canonical identity is `(provider, absolute_path)` and `session_id` stays non-unique, resolved newest-timestamp-first. Cursor transcripts may also set `isImportedFromClaude` / `isImportedFromCodex` when the file is a copy of another provider (schema v8); `isImportedFromCursor` is reserved for Claude/Codex providers.
...
- `isCodexInjectedContext()` (public export) — the Codex-preamble predicate `reduceCodexEntry` uses to skip AGENTS.md / sandbox blobs. Exported because `parseCodexJsonlLine` is stateless and so **cannot** apply it: the skip is bounded to the LEADING turn, and one line does not know its position. A downstream indexer numbers messages itself, so it owns that bound — without this it would index in a different space than anything rendering from the same file
